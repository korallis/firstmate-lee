...
The default bare-prompt regex is written as `^(❯|›)`, not a UTF-8 bracket expression, because `grep -E` under `LC_ALL=C` can otherwise match unrelated box-drawing bytes and mistake decorative rows for bare prompt rows.
...
The default uses `^(❯|›)` alternation instead of a bracket expression so `LC_ALL=C` does not treat UTF-8 prompt glyphs bytewise and match box-drawing rows by accident.
...
The locale regression is covered by `test_composer_state_c_locale_keeps_box_rows_out_of_bare_prompt_match` and `test_composer_state_c_locale_strips_utf8_prompt_prefix`.
