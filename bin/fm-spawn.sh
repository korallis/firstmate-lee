...
#   harness (config/secondmate-harness -> config/crew-harness -> own dispatchable
#   harness), so the secondmate-vs-crewmate split is DURABLE across every respawn
#   (recovery, /updatefirstmate, restart). A bare adapter name (claude|codex|opencode|pi|grok)
...
case "$ARG3" in
  *' '*)  # raw launch command (unverified-adapter escape hatch)
  ...
  '')
    ...
    # secondmate harness (config/secondmate-harness -> config/crew-harness ->
    # own dispatchable harness);
